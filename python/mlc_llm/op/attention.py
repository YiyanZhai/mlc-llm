...
def attention(  # pylint: disable=invalid-name,too-many-locals,too-many-statements,too-many-arguments
    ...
    def _fallback():
        from mlc_llm.nn import _attention_sequence_prefill # pylint: disable=import-outside-toplevel, cyclic-import
        ...
        attn_output, _ = op.tensor_ir_op(
            _attention_sequence_prefill(h_kv, h_q, d, q.dtype, target),
            "sequence_prefill",
            [q, k, v],
            [
        ...
        attn_output = op.reshape(attn_output, shape=(b, s, h_q, d))
